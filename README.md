# 👋 Hola, soy **Oscar Serrano**

🎓 Estudiante de **Ingeniería de Sistemas** en las **Unidades Tecnológicas de Santander (UTS)**  
💻 Apasionado por el **desarrollo web**, **Java**, bases de datos y la creación de soluciones funcionales  
🚀 Construyendo mi perfil como **ingeniero de sistemas** mediante proyectos reales, buenas prácticas y aprendizaje continuo  

🧠 Me enfoco en **entender cómo funcionan las cosas por dentro**, no solo en que “funcionen”.

---

## 🚀 Tech Stack

### 🏆 Tecnologías que domino
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwindcss&logoColor=white)
![Java](https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=java&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)

### 💡 Tecnologías con nivel medio
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Maven](https://img.shields.io/badge/Maven-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white)

### 🌿 Experiencia adicional
![Bootstrap](https://img.shields.io/badge/Bootstrap-563D7C?style=for-the-badge&logo=bootstrap&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Tinkercad](https://img.shields.io/badge/Tinkercad-FF6F00?style=for-the-badge&logo=autodesk&logoColor=white)
![APIs](https://img.shields.io/badge/APIs-000000?style=for-the-badge&logo=postman&logoColor=white)

📌 Me siento cómodo creando **CRUDs en Java**, conectando aplicaciones a **PostgreSQL**, aplicando **DAO / MVC** y desarrollando **interfaces web funcionales, limpias y bien estructuradas**.

---

## 🧠 Enfoque Técnico (Mentalidad de Ingeniería)

- Modelado de bases de datos (MER, normalización)
- Arquitectura **DAO / MVC**
- Separación de responsabilidades
- Código legible y mantenible
- Uso consciente de Git (commits claros, versionado)
- Seguridad básica: contraseñas hasheadas, validaciones, control de acceso

---

## 📚 Formación destacada (UTS)

- **Motores y Diseño de Bases de Datos**
  - PostgreSQL (DDL, DML)
  - Funciones, procedimientos, auditoría y seguridad
- **Programación Orientada a Objetos con Java**
- **Diseño Web Avanzado**
  - HTML, CSS, JavaScript, Tailwind y Bootstrap
- **Programación de dispositivos con C++ en Tinkercad**
- **Certificado:** Solución de problemas mediante algoritmos

---

## 🌐 Proyectos Reales — Desplegados en Vercel

### 🛒 Tienda Online — E-commerce Académico
📦 Gestión de productos y carrito de compras  
🛠️ HTML | CSS | JavaScript  

🔗 **Demo**  
https://tienda-online-dusky.vercel.app/

📁 **Repositorio**  
https://github.com/oserrano12/tienda_online

---

### ☕ Proyecto Final — Sabores Bucaramanga
Aplicación tipo cafetería  
💻 HTML | CSS | JS | Tailwind  
👥 Proyecto colaborativo universitario  

🔗 **Demo**  
https://proyecto-final-one-snowy.vercel.app/

📁 **Repositorio**  
https://github.com/oserrano12/proyecto-final

---

### ☕ Café Artesanal App (Versión preliminar)
Primer prototipo del proyecto final  

🔗 **Demo**  
https://cafe-artesanal-app.vercel.app/

📁 **Repositorio**  
https://github.com/oserrano12/cafe-artesanal-app

---

## 🧪 Proyecto Backend en Desarrollo

### 🏪 Sistema de Gestión — Tienda de Barrio
Proyecto backend académico construido **desde cero** en **Java + PostgreSQL**, sin frameworks, enfocado en arquitectura y buenas prácticas.

🛠️ Java | JDBC | PostgreSQL | DAO | MVC | Git  

📌 Enfoque técnico:
- Modelado de base de datos relacional
- CRUD completo de entidades
- Arquitectura DAO / MVC
- Separación clara por capas
- Seguridad básica (hash de contraseñas, roles)
- Versionado con Git y ramas de trabajo

📁 **Repositorio**  
https://github.com/oserrano12/tienda-barrio

🚧 Proyecto actualmente **en desarrollo activo**

---

## 📊 GitHub Stats

> Actividad real de código y constancia de aprendizaje

![Oscar's GitHub stats](https://github-readme-stats.vercel.app/api?username=oserrano12&show_icons=true&theme=tokyonight&hide_border=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=oserrano12&layout=compact&theme=tokyonight&hide_border=true)

![GitHub Streak](https://streak-stats.demolab.com?user=oserrano12&theme=tokyonight&hide_border=true)

---

## 🔧 Entorno de Desarrollo
- IntelliJ IDEA — Java + Maven
- VS Code — desarrollo web
- Git + GitHub — control de versiones
- PostgreSQL + pgAdmin
- Linux — entorno de práctica
- Tinkercad — simulaciones con C++

---

## 📈 Objetivo Actual

🔥 **Backend con Java**
- CRUDs con JDBC
- PostgreSQL avanzado
- Arquitectura DAO / MVC
- Buenas prácticas de seguridad
- Preparación para interfaces gráficas (Swing)
- Introducción a APIs REST

---

## 🗺️ Roadmap Personal

- [x] Fundamentos sólidos de Java
- [x] Bases de datos relacionales
- [x] CRUDs completos
- [ ] Aplicación Java + Swing
- [ ] API REST con Java
- [ ] Autenticación y roles
- [ ] Proyecto backend completo desplegable

---

## 📫 Contacto
- **GitHub**  
https://github.com/oserrano12  

- **Discord**  
**deadly04**
